...
export const generateInitialValues = (camper: ICamper): Partial<IFormikCamper> => {
  const { first_name, last_name, playa_name, city, role, about_me, history, social_links, tags, visitedBM, birthdayDate, createdBy } = camper;
  ...
  return {
    role,
    createdBy,
    custom_email: camper.custom_email || '',
