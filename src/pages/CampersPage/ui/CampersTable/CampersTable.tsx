    ...
    () => [
      {
        ...
        cell: info => {
          ...
          const canDelete = isCurrentUserTCO ? !currentUser : isCurrentUserCOORG ? !isTCO && (currentUser || !isCOORG) : currentUser;

          return (
            <div className={classNames(styles.table__row, {}, [styles.wrap])}>
              <p className={styles.table__camperName} onClick={() => handleOpenDetails({ email, detailsTheme: CamperDetailsModalTheme.DEFAULT })}>
                {firstName} {lastName}
              </p>
      ...
      },
      {
        ...
        cell: info => <TruncatedText text={info.getValue() as string} maxLength={isExtraLargePCBreakpoint ? 15 : 25} nowrap />,
      },
      {
        ...
        cell: info => (info.getValue() ? dateFormatter(info.getValue() as string, 'withoutYear') : ''),
      },
      {
        accessorKey: 'role',
        header: 'Role',
      ...
      },
      {
        ...
        cell: info => <TruncatedText text={info.getValue() as string} maxLength={isExtraLargePCBreakpoint ? 10 : 20} nowrap />,
      },
      {
      ...
      },
      {
        ...
        cell: info => (info.getValue() ? dateFormatter(info.getValue() as string) : ''),
      },
    ],
  ...
  return (
    <>
      <Table<ICamper>
        ...
        isCreateButton
      />
      <CamperDetailsModal camperEmail={camperEmail} theme={detailsTheme} isOpen={isOpen} onClose={close} />
    </>
  );
};
