...
export const getTestParameters = async (): Promise<{
  ...
  const [TEST_COGNITO_POOL_ID, COGNITO_APP_CLIENT_ID, TEST_EMAIL, TEMP_PASSWORD, NEW_PASSWORD, CAMP_ID, APP_URL] = await Promise.all([
...
export const login = async (page: Page, URLS: Record<string, string>, TEST_PARAMS: Record<string, string>) => {
...
export const generateFakeData = () => {
  ...
  const password = faker.string.alphanumeric(6) + faker.string.numeric(1) + faker.string.symbol(1) + faker.string.alpha(1).toUpperCase();
