...
export const registrationSchema = yup.object().shape({
  ...
  role: yup.string(),
  city: yup.string().trim().min(2, 'City name must be at least 2 characters').max(32, 'Less than 32 characters').required('City is required'),
...
export const loginSchema = yup.object().shape({
  email: yup.string().trim().email('Invalid email address').required('Email is required'),
  password: yup.string().trim().min(8, 'Minimum 8 character').max(40, 'Maximum 40 characters').required('Password is required'),
});
...
export const camperRegistrationSchema = yup.object().shape({
  ...
  last_name: yup.string().trim().required('Field is required').max(32, 'Last name must be less than 32 characters'),
  playa_name: yup.string().trim().max(24, 'Less than 24 characters'),
  about_me: yup.string().trim().max(256, 'Less than 256 characters'),
  email: yup.string().trim().email('Invalid email address').required('Email is required'),
  password: yup
    .string()
    .trim()
...
export const campSettingsSchema = yup.object().shape({
  camp_name: yup
    ...
    .max(32, 'Less than 32 characters'),
  city: yup.string().trim().min(2, 'City name must be at least 2 characters').max(32, 'Less than 32 characters').required('City is required'),
  camp_website: yup.string().trim().matches(urlRegex, 'Invalid website address'),
...
export const editCamperSchema = yup.object().shape({
  ...
  city: yup.string().trim().max(32, 'Less than 32 characters'),
  about_me: yup.string().trim().max(256, 'Less than 256 characters'),
  role: yup.mixed<CamperRole>().oneOf(Object.values(CamperRole) as CamperRole[], 'Invalid role selected'),
  tags: yup
    .array()
    .of(
      yup.object().shape({
        tagName: yup
          .string()
          .trim()
          .test('tagName-required-if-tagDetails', 'Tag name is required', function (value, context) {
...
export const shiftSchema = yup.object().shape({
  title: yup.string().trim().required('Title is required').max(32, 'Title must be less than 64 characters'),
  description: yup.string().trim().max(255, 'Description must be less than 255 characters'),
  members: yup.array().of(yup.string().trim().required('Member name is required')).min(1, 'At least one member is required'),
    ...
    .of(
      yup.object().shape({
        ...
        end_time: yup
          .date()
          .nullable()
          .when('start_time', (start_time, schema) => (start_time[0] ? schema.min(start_time, 'End time cannot be before start time') : schema)),
      })
    )
...
export const createCalendarEventSchema = yup.object().shape({
  event: yup.string().trim().required('Title is required').max(24, 'Title must be less than 24 characters'),
